function [Je,flag,relres,iter,resvec,solvert,vsol] = JVIE_Solver_const_GPU(Einc,e_r,r,freq,fN,tol,form,solver,precond,max_it,inner_it,outer_it,ritz,gpu_flag,logfile)
    ...
    if (gpuDeviceCount) && (gpu_flag)  % check GPU existence and user opt
        ...
        if (dev.TotalMemory > memestimated ) % verify memory of GPU and problem size
            ...
            try
                ...
                % define the main function for the matrix vector product
                fAGPU   = @(J,transp_flag)mv_AN_const_GPU(J, fN_gpu, Mr_gpu, Mc_gpu, Gram, transp_flag, idxS3, 1);
                % define the preconditioner
                fPGPU   = @(J,transp_flag)mv_AN_const(J, fN_gpu, 1./Mr_gpu, (1./Mr_gpu - 1.0), Gram, transp_flag, idxS3, 1);
