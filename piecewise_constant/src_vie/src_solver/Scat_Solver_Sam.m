function [Jout,Sout,Eout,Bout,Gsar,Pabs,fN,fK] = Scat_Solver_Sam(Einc,Hinc,freq,r,epsilon_r,sigma_e,fN,fK,Jeout,rho, tol, form,solver,precond,max_it,inner_it,outer_it,ritz,gpu_flag,logfile)
...
if(nargin < 7 || isempty(fN))
   fN = [];
end
if(nargin < 8 || isempty(fK))
   fK = [];
end
if(nargin < 10 || isempty(form))
   form = 2;
end
if(nargin < 11 || isempty(solver))
   solver = 'M';
else
...
if(nargin < 15 || isempty(max_it))
   max_it = 1000;
end
if(nargin < 16 || isempty(inner_it) || (inner_it < 1))
   inner_it = 50;
end
if(nargin < 17 || isempty(outer_it) || (outer_it < 1))
   outer_it = 200;
end
if(nargin < 18 || isempty(ritz) || (ritz < 1))
   ritz = 5;
end
if(nargin < 19 || isempty(gpu_flag))
   gpu_flag = 1;
end

if(nargin < 20 || isempty(logfile))
   fid = 1;
   logfile = [];
else
    fid = fopen(logfile, 'a');
...
%m_r = mu_r - 1j*sigma_m/(mu_o*omega);
...
t1 = tic;
        ...
        [Jout,~,~,~,~,~] = JVIE_Solver_const_Sam(Einc,e_r,r,freq,fN,Jeout,tol,form,solver,precond,max_it,inner_it,outer_it,ritz,gpu_flag,logfile);
...
t1 = tic;
        ...
        [Eout] = E_field_Nop_const(Jout,fN,Gram,freq,Einc);
...
t1 = tic;
        ...
        [Bout] = H_field_Kop_const(Jout,fK,Gram,Hinc);
