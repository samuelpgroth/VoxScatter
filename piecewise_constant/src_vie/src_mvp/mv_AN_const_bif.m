function [JOut] = mv_AN_const_bif(JIn0, fN, Mr_x, Mr_y, Mr_z,...
    Mc_x, Mc_y, Mc_z, Gram, transp_flag, idx, GPU_flag)
...
if strcmp(transp_flag,'transp')       % JOut = fN' * JIn0
    ...
    Jout1 = ifftn(Jout1);
    JOut(:,:,:,1) = Gram .* conj(Mr_x) .* JIn(:,:,:,1) - conj( Jout1(1:L,1:M,1:N));
    Jout2 = ifftn(Jout2);
    JOut(:,:,:,2) = Gram .* conj(Mr_y) .* JIn(:,:,:,2) - conj( Jout2(1:L,1:M,1:N));
    Jout3 = ifftn(Jout3);
    JOut(:,:,:,3) = Gram .* conj(Mr_z) .* JIn(:,:,:,3) - conj( Jout3(1:L,1:M,1:N));
...
elseif strcmp(transp_flag,'notransp') % JOut = fN * JIn0
    ...
    % apply ifft, multiply by material properties and Gram
    Jout1 = ifftn(Jout1);
    JOut(:,:,:,1) = Gram .* Mr_x .* JIn(:,:,:,1) - Mc_x .* Jout1(1:L,1:M,1:N);
    Jout2 = ifftn(Jout2);
    JOut(:,:,:,2) = Gram .* Mr_y .* JIn(:,:,:,2) - Mc_y .* Jout2(1:L,1:M,1:N);
    Jout3 = ifftn(Jout3);
    JOut(:,:,:,3) = Gram .* Mr_z .* JIn(:,:,:,3) - Mc_z .* Jout3(1:L,1:M,1:N);
