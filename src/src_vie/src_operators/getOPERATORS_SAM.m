function [op_out,opToeplitz] = getOPERATORS_SAM(r,f,op,type,singular)
...
% Domain & Frequency
% -------------------------------------------------------------------------
% dx = r(1,1,2,3) - r(1,1,1,3); % voxel edge length NB: changed from r(2,1,1,1)-r(1,1,1,1)
...
if strcmp(op ,'N')
    ...
    if strcmp(singular ,'DIRECTFN')
        [opToeplitz] = assembly_nop_const(r,ko,dx,'DIRECTFN');
    elseif strcmp(singular ,'DEMCEM')
        [opToeplitz] = assembly_nop_const(r,ko,dx,'DEMCEM');
    end
    
    % Type
    if strcmp(type ,'FFTofC')
        opCirculant = circulant_nop_const(opToeplitz); 
        op_out = fft_operator(opCirculant); 
    elseif strcmp(type ,'T') 
        op_out = opToeplitz;           
    elseif strcmp(type ,'C')
        op_out = circulant_nop_const(opToeplitz);    
    end
...
if strcmp(op ,'K')
    ...
    [opToeplitz] = assembly_kop_const(r,ko,dx);
    
    % Type
    if strcmp(type ,'FFTofC')
        opCirculant = circulant_kop_const(opToeplitz); 
        op_out = fft_operator(opCirculant); 
    elseif strcmp(type ,'T') 
        op_out = opToeplitz;           
    elseif strcmp(type ,'C')
        op_out = circulant_kop_const(opToeplitz);    
    end
