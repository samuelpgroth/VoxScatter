function [Je,flag,relres,iter,resvec,solvert] = JVIE_Solver_const_Sam(Einc,e_r,r,freq,fN,Jtemp,tol,form,solver,precond,max_it,inner_it,outer_it,ritz,gpu_flag,logfile)
...
if(nargin < 11 || isempty(max_it))
   max_it = 1000;
end
if(nargin < 12 || isempty(inner_it) || (inner_it < 1))
   inner_it = 50;
end
if(nargin < 13 || isempty(outer_it) || (outer_it < 1))
   outer_it = 200;
end
if(nargin < 14 || isempty(ritz) || (ritz < 1))
   ritz = 5;
end
if(nargin < 15 || isempty(gpu_flag))
   gpu_flag = 1;
end

if(nargin < 16 || isempty(logfile))
   fid = 1;
else
    fid = fopen(logfile, 'a');
    if (fid == -1)
...
% Compute the relative permittivity and susceptibility
Mr = e_r;
Mc = e_r - 1.0;
...
[Jknown_MVprod] = mv_AN_const(Jknown, fN, Mr, Mc, Gram, 'notransp', idxS3, 0);

Vrhs = Vrhs - Jknown_MVprod;
% Vrhs = - Jknown_MVprod;
...
if (form == 1) 
    ...
    if (gpuDeviceCount) && (gpu_flag)  % check GPU existence and user opt
        ...
        if (dev.TotalMemory > memestimated ) % verify memory of GPU and problem size
            ...
            try
                ...
                fPGPU   = @(J,transp_flag)mv_AN_const(J, fN_gpu, 1./Mr_gpu, (1./Mr_gpu - 1.0), Gram, transp_flag, idxS3, 1);
    ...
    % define the preconditioner
    fPCPU   = @(J,transp_flag)mv_AN_const(J, fN, 1./Mr, (1./Mr - 1.0), Gram, transp_flag, idxS3, 0);
