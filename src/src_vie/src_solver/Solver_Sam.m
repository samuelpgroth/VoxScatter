function [Je,flag,relres,iter,resvec,solvert,vsol] = Solver_Sam(Einc,e_r,r,freq,fN,tol,form,solver,precond,max_it,inner_it,outer_it,ritz,gpu_flag,logfile)
...
dx = r(1,1,2,3)-r(1,1,1,3); %NB: I changed this from original dx=r(2,1,1,1)-r(1,1,1,1); Shouldn't make a difference.
Gram = dx^3;
...
% -------------------------------------------------------------------------
    ...
    Mc(:) = Mc(:)./Mr(:);
    % HACK: below introduced squeezes since for the one voxel case, the
    % Mr's wouldn't automatically squeeze
    if L==1
    ...
    if (gpuDeviceCount) && (gpu_flag)  % check GPU existence and user opt
        ...
        if (dev.TotalMemory > memestimated ) % verify memory of GPU and problem size
            ...
            try
                ...
                fAGPU   = @(J,transp_flag)mv_AN_const_GPU(J, fN_gpu, Mr_gpu, Mc_gpu, Gram, idxS3);
               % fAGPU   = @(J,transp_flag)mv_AN_const(J, fN_gpu, Mr_gpu, Mc_gpu, Gram, transp_flag, idxS3, 1);
    ...
    % define the main function for the matrix vector product
    fACPU   = @(J,transp_flag)mv_AN_const(J, fN, Mr, Mc, Gram, transp_flag, idxS3, 0);
 %keyboard
%     tic
%     for i=1:100
%        hey=fACPU(Vrhs,'notransp');
%     end
%     toc
%     
%     tic
...
%     for i=1:100
%        % VRHS = gpuArray(Vrhs);
%         ya=fAGPU(Vrhs,'notransp');
%         %YA=gather(ya);
%     end
%     
%     toc
    
    %[vsol, flag, relres, iter, resvec] = gmres(@(J)fACPU(J,'notransp'), Vrhs, inner_it, tol, outer_it);
%     tic
%     [X,FLAG,RELRES,ITER,RESVEC] = gmres(@(J)fACPU(J,'notransp'), Vrhs,50,1e-4,1000);
%     toc
    
%     tic
%     VRHS = gpuArray(Vrhs);
%     [XX,FLAG,RELRES,ITER,RESVEC] = gmres(@(J)fAGPU(J,'notransp'), VRHS,200,1e-4,1000);
%     x=gather(XX);
%     toc
    
%     tic
%     [vsol, flag, relres, iter, resvec] = pgmres(@(J)fACPU(J,'notransp'), Vrhs, 200, tol, outer_it);
%     toc
...
%       [vsol, flag, relres, iter, resvec] = gmres(@(J)fACPU(J,'notransp'), Vrhs, inner_it, tol, max_it);
        ...
        try
           % [vsol, flag, relres, iter, resvec] = pgmres(@(J)fAGPU(J,'notransp'), Vrhs, inner_it, tol, outer_it);
            ...
            VRHS = gpuArray(Vrhs);
%            [X,flag,relres,iter,resvec] = gmres(@(J)fAGPU(J,'notransp'), VRHS, inner_it, tol, max_it);
             [X, flag, relres, iter, resvec] = bicgstab(@(J)fAGPU(J,'notransp'), VRHS, tol, max_it);
            vsol=gather(X);
